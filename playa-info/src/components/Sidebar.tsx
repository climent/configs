...
export default function Sidebar({
  ...
  const [activeTab, setActiveTab] = useState<'build' | 'metadata' | 'decks'>('build');
  ...
  const handleDragStartElement = (e: React.DragEvent, elementType: ElementType) => {
  ...
  const handleDragStartTemplate = (e: React.DragEvent, templateType: string) => {
    e.dataTransfer.setData('text/plain', JSON.stringify({
      dragType: 'template',
  ...
  const handleExportDecksJSON = () => {
    try {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(presentations, null, 2));
      const downloadAnchor = document.createElement('a');
      downloadAnchor.setAttribute("href", dataStr);
      downloadAnchor.setAttribute("download", `slide-builder-backup-${new Date().toISOString().split('T')[0]}.json`);
      ...
      fileReader.onload = (event) => {
        try {
          ...
          if (Array.isArray(parsed)) {
            ...
            alert('Invalid backup file format. Must be a JSON array of presentations.');
          }
        } catch (err) {
  ...
  const isArchived = activeMeta ? (activeMeta.expiresAt ? new Date(activeMeta.expiresAt).getTime() <= currentTime.getTime() : false) : false;

  return (
    <div id="sidebar-container" className="w-80 border-r border-slate-200 bg-white flex flex-col h-full shrink-0 select-none shadow-sm z-10">
      {/* App Header */}
      <div id="app-header" className="p-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-indigo-50 text-indigo-600 rounded-xl shadow-inner animate-pulse">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <h1 className="font-sans font-bold text-slate-900 tracking-tight text-sm">Slide PDF Builder</h1>
            <p className="font-sans text-[10px] text-slate-500 font-semibold tracking-wider uppercase">ACCESSIBLE 20PT+ STANDARDS</p>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div id="sidebar-tabs" className="flex border-b border-slate-100 px-2 bg-slate-50/50">
        <button
          id="tab-build"
      ...
      {/* Tab Contents */}
      <div id="sidebar-tab-content" className="flex-1 overflow-y-auto p-4 space-y-5">
        {activeTab === 'build' && (
          <>
            {isArchived && (
              <div id="expiry-warning-banner" className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
                <Lock className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                <div className="text-[11px] text-amber-800 leading-normal">
                  <span className="font-bold block">Document Archived</span>
                  This deck is expired. Editing is disabled until you update the expiration settings.
                </div>
              </div>
            )}
            ...
            {/* Draggable Templates */}
            <div>
              <h3 id="heading-templates" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                <FolderOpen className="w-3.5 h-3.5 text-indigo-500" /> Layout Templates
              </h3>
              <p className="text-[11px] text-slate-500 mb-3.5 leading-relaxed">
                Drag a layout template onto the active slide workspace to instantly apply the layout.
              </p>
              <div id="templates-list" className="grid grid-cols-2 gap-2">
                <div
                  ...
                  className={`border border-slate-150 bg-slate-50 hover:bg-slate-100 p-2.5 rounded-xl flex flex-col justify-between h-20 transition-all select-none hover:shadow-sm ${
                    isArchived ? 'opacity-50 cursor-not-allowed' : 'cursor-grab active:cursor-grabbing hover:border-slate-300'
                  }`}
                >
                  <span className="text-[11px] font-bold text-slate-700">Welcome Slide</span>
                ...
                >
                  <span className="text-[11px] font-bold text-slate-700">Bullet Points</span>
                  <div className="w-full h-4 bg-white border border-slate-200 rounded flex flex-col gap-0.5 justify-center px-1.5">
                    <div className="w-5 h-1 bg-indigo-500 rounded-full" />
                    <div className="flex items-center gap-1"><div className="w-1 h-1 bg-slate-400 rounded-full shrink-0" /><div className="w-6 h-0.5 bg-slate-300 rounded-full" /></div>
                    <div className="flex items-center gap-1"><div className="w-1 h-1 bg-slate-400 rounded-full shrink-0" /><div className="w-4 h-0.5 bg-slate-300 rounded-full" /></div>
                  </div>
                </div>
                ...
                >
                  <span className="text-[11px] font-bold text-slate-700">Stats</span>
                ...
                >
                  <span className="text-[11px] font-bold text-slate-700">Quote</span>
                  <div className="w-full h-4 bg-white border border-slate-200 rounded flex items-center gap-1 px-1.5">
                ...
                >
                  <span className="text-[11px] font-bold text-slate-700">2-Column Info Grid</span>
            ...
            <div>
              <h3 id="heading-elements" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                <Plus className="w-3.5 h-3.5 text-indigo-500" /> Add Slide Elements
              </h3>
              <p className="text-[11px] text-slate-500 mb-3">
                Drag a dynamic block into the slide container or click to append.
              </p>
              <div id="elements-list" className="space-y-1.5">
                <div
                  ...
                  className={`flex items-center gap-3 p-2 bg-white hover:bg-indigo-50/40 border border-slate-150 rounded-xl text-xs font-semibold text-slate-700 hover:border-indigo-200 hover:text-indigo-700 transition-all select-none hover:shadow-sm ${
                    isArchived ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer active:scale-98'
                  }`}
                >
            ...
            {/* Quick Themes */}
            <div>
              <h3 id="heading-themes" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-indigo-500" /> Apply Slide Theme
              </h3>
              <p className="text-[11px] text-slate-500 mb-3">
                Instantly style the active slide's background, colors, and font layout.
              </p>
                ...
                {DEFAULT_THEMES.map((theme, idx) => (
                  <button
                    ...
                    className={`flex items-center justify-between p-2 rounded-xl border border-slate-200 transition-all hover:border-slate-400 bg-white text-left ${
                      isArchived ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer hover:shadow-sm'
                    }`}
                  >
                    <div className="flex items-center gap-1.5 truncate">
                      <span className="text-[11px] font-semibold text-slate-800 truncate">{theme.fontFamily}</span>
                    </div>
                    <div className="flex gap-1 shrink-0">
                      <span className="w-3 h-3 rounded-full border border-slate-200 shrink-0 shadow-inner" style={{ backgroundColor: theme.background }} />
                      <span className="w-3 h-3 rounded-full border border-slate-200 shrink-0 shadow-inner" style={{ backgroundColor: theme.accent }} />
                    </div>
                  </button>
                ))}
              </div>
        ...
        {activeTab === 'metadata' && activeMeta && (
          <div id="metadata-settings" className="space-y-4">
            <div>
              <h3 id="metadata-header" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                Document Metadata
              </h3>
              <p className="text-[11px] text-slate-500 mb-3.5 leading-relaxed">
                This metadata will be bundled inside the document and manages slide archiving constraints.
              </p>
            </div>

            <div className="space-y-3">
              <div>
                <label id="label-meta-title" className="block text-[11px] font-bold text-slate-600 uppercase mb-1">PRESENTATION TITLE</label>
                <input
                  id="input-meta-title"
                  type="text"
              ...
              <div>
                <label id="label-meta-dept" className="block text-[11px] font-bold text-slate-600 uppercase mb-1">DEPARTMENT</label>
                <input
                  id="input-meta-dept"
                  type="text"
                  value={activeMeta.department}
                  onChange={(e) => onUpdateMetadata({ department: e.target.value })}
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:bg-white focus:border-indigo-500 transition-all"
              ...
              <div>
                <label id="label-meta-ref" className="block text-[11px] font-bold text-slate-600 uppercase mb-1">CREATOR NAME</label>
                <input
                  id="input-meta-ref"
                  type="text"
                  value={activeMeta.referenceNumber}
                  onChange={(e) => onUpdateMetadata({ referenceNumber: e.target.value })}
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:bg-white focus:border-indigo-500 transition-all"
              ...
              <div>
                <label id="label-meta-creator" className="block text-[11px] font-bold text-slate-600 uppercase mb-1">PUBLISHED DATE</label>
                <input
                  id="input-meta-creator"
                  type="text"
                  value={activeMeta.creator}
                  onChange={(e) => onUpdateMetadata({ creator: e.target.value })}
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:bg-white focus:border-indigo-500 transition-all"
              ...
              <div className="border-t border-slate-100 pt-3">
                <div className="flex items-center justify-between mb-1.5">
                  <label id="label-meta-expiry" className="block text-[11px] font-bold text-slate-600 uppercase">DOCUMENT EXPIRATION</label>
                ...
                <p className="text-[10px] text-slate-500 mb-2.5 leading-normal">
                  Set a date and time for this file to be thrown away. The date gets saves alongside the file, and it is used by the system to automatically delete it. Use this when you are creating slides with information that expires, like Ranger social events.
                </p>
                ...
                <div className="flex items-center gap-1.5 mb-2.5">
                  ...
                  <input
                    ...
                    type="datetime-local"
                    value={activeMeta.expiresAt ? new Date(new Date(activeMeta.expiresAt).getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().slice(0, 16) : ''}
                    onChange={(e) => {
                      if (e.target.value) {
                        onUpdateMetadata({ expiresAt: new Date(e.target.value).toISOString() });
                      } else {
                ...
                <div className="space-y-1.5">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">QUICK EXPIRY PRESETS (FOR PROTOTYPE REVIEW):</span>
                  <div id="expiry-presets-list" className="grid grid-cols-4 gap-1">
                    <button
                      id="expiry-preset-1h"
                ...
                <div id="metadata-expiry-countdown-display" className="mt-3.5 bg-slate-50 border border-slate-150 rounded-xl p-2.5 flex items-center justify-between text-[11px]">
                  <span className="text-slate-500 font-semibold">Archive Status:</span>
                  <div className="flex items-center gap-1.5 font-bold">
                    <Clock className={`w-3.5 h-3.5 ${isArchived ? 'text-amber-500' : 'text-emerald-500 animate-pulse'}`} />
                    <span className={isArchived ? 'text-amber-700' : 'text-emerald-700'}>
        ...
        {activeTab === 'decks' && (
          <div id="decks-tab-container" className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-500 uppercase">Available Decks</span>
              <button
            ...
            <div id="saved-decks-list" className="space-y-2">
              {presentations.map((pres) => {
                ...
                const isPresArchived = pres.metadata.expiresAt ? new Date(pres.metadata.expiresAt).getTime() <= currentTime.getTime() : false;
                  ...
                  >
                    ...
                    <div className="mt-2.5 pt-2 border-t border-slate-100 flex items-center justify-between text-[10px]">
                      <div className="flex items-center gap-1">
                        {isPresArchived ? (
                          <span id={`deck-status-archived-${pres.id}`} className="inline-flex items-center gap-0.5 bg-amber-50 text-amber-700 font-bold px-1.5 py-0.5 rounded border border-amber-200">
                            <Lock className="w-2.5 h-2.5" /> Archived
                          </span>
                        ) : (
                          <span id={`deck-status-active-${pres.id}`} className="inline-flex items-center gap-0.5 bg-emerald-50 text-emerald-700 font-bold px-1.5 py-0.5 rounded border border-emerald-200">
                            <CheckCircle2 className="w-2.5 h-2.5" /> Active
                          </span>
                        )}
                      </div>
                      <span className="text-slate-400 font-medium truncate max-w-[120px]">
                        {pres.metadata.expiresAt ? formatTimeRemaining(pres.metadata.expiresAt) : 'Never expires'}
                      </span>
                    </div>
                  </div>
                );
            ...
            <div className="pt-3 border-t border-slate-100 space-y-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Backup & Recovery</span>
              <p className="text-[10px] text-slate-500 leading-normal">
                Your decks are safely cached in this browser. Export a backup file to keep them secure or move them to another machine.
              </p>
      ...
      <div id="accessibility-notice" className="p-4 border-t border-slate-100 bg-slate-50/50">
        <div className="flex gap-2.5">
          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-700 uppercase block tracking-wider">Accessibility Constraint</span>
            <span className="text-[10px] text-slate-500 leading-normal block">
              Slides strictly enforce a minimum font size of <b>20pt (20px)</b> to guarantee high readability and compliance for public information displays.
            </span>
          </div>
        </div>
      </div>
