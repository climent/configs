...
import { Slide, SlideElement, SlideTheme, ElementType, MIN_FONT_SIZE } from '../types';
...
export default function SlideCanvas({
  ...
  return (
    <div id="canvas-workspace" className="flex-1 bg-slate-50 flex flex-col items-center justify-between p-6 overflow-y-auto overflow-x-hidden select-none h-full">
      {/* Top action bar */}
      <div id="canvas-header" className="w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl flex items-center justify-between mb-2">
        <div className="flex flex-col">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Interactive Preview</span>
      ...
      <div className="w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl flex-1 flex items-center justify-center relative min-h-[360px] my-2">
        ...
        {isOverflowing && (
          <div id="canvas-overflow-alert" className="absolute top-4 left-1/2 -translate-x-1/2 z-30 w-11/12 max-w-md bg-amber-50/95 backdrop-blur border border-amber-200 rounded-xl p-2.5 flex items-center gap-2.5 shadow-lg text-amber-800 transition-all">
            <div className="p-1 bg-amber-100 rounded-lg text-amber-600 animate-bounce shrink-0">
              <AlertTriangle className="w-4 h-4" />
            </div>
            <div className="flex flex-col text-left">
              <span className="text-xs font-bold leading-none mb-0.5">Content Exceeds Slide Canvas Bounds</span>
              <p className="text-[10px] text-amber-600 leading-normal">
                An element (e.g. paragraph or bullet list) is overflowing the available slide area. Consider shrinking its font size, adjusting alignments, or reducing content to maintain clear layout readability.
              </p>
            </div>
          </div>
        )}
        ...
        <div
          ...
          style={{
            ...
            color: theme.text,
            border: theme.cardStyle === 'bordered' ? '2px solid rgba(0,0,0,0.08)' : 'none',
            boxShadow: theme.cardStyle === 'shadow' ? '0 25px 50px -12px rgba(0,0,0,0.15)' : 'none',
          }}
        >
          {/* Metadata Header Band */}
          <div id="slide-metadata-header" className="flex items-center justify-between text-[11px] font-semibold uppercase opacity-60 tracking-wider mb-2 border-b pb-2" style={{ borderColor: `${theme.accent}30` }}>
            <span className="truncate max-w-[200px]">{metaDept || 'DEPARTMENT'}</span>
            <span className="truncate max-w-[200px]">{metaRef || 'CREATOR NAME'}</span>
          </div>

          {/* Core Content Elements Workspace Stack */}
          <div id="slide-element-stack" ref={elementStackRef} className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden min-h-0 select-none scrollbar-thin px-3">
            <div className="flex flex-col gap-4 py-3 my-auto w-full">
              {elements.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-8 transition-all min-h-[160px]" style={{ borderColor: `${theme.accent}30` }}>
                  <MousePointerClick className="w-8 h-8 opacity-40 mb-2 text-indigo-500 animate-bounce" />
                  <span className="text-xs font-bold uppercase tracking-wide opacity-50">Drag Template or Elements here</span>
                  <span className="text-[10px] opacity-40 text-center max-w-[260px] mt-1">Use the left builder sidebar to layout your accessibility-locked slide content.</span>
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      {el.type === 'paragraph' && (
                        ...
                        >
                          {el.content || 'Insert clear narrative slides here. Enforcing a minimum text size of 20pt ensures perfect legibility for distant presentation displays.'}
                        </p>
                      )}

                      {el.type === 'bulletList' && (
                        <ul id={`render-bullets-${el.id}`} className="space-y-2.5 list-none" style={{ textAlign: el.align }}>
                          {(el.listItems || []).map((item, bIdx) => (
                            ...
                            >
                              <span className="shrink-0 font-bold" style={{ color: theme.accent }}>•</span>
                              <span className="font-semibold">{item || 'Empty bullet item'}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                      ...
                      {el.type === 'quote' && (
                        <div id={`render-quote-${el.id}`} className="flex gap-4 py-1" style={{ justifyContent: el.align === 'center' ? 'center' : el.align === 'right' ? 'flex-end' : 'flex-start' }}>
                          <div className="w-1.5 rounded shrink-0" style={{ backgroundColor: theme.accent }} />
                          <div
                          ...
                          >
                            "{el.content || 'Designing for accessibility guarantees that we reach everyone.'}"
                          </div>
                        </div>
                      )}

                      {el.type === 'stat' && (
                        <div id={`render-stat-${el.id}`} className="flex flex-col" style={{ alignItems: el.align === 'center' ? 'center' : el.align === 'right' ? 'flex-end' : 'flex-start' }}>
                          <div
                      ...
                      {el.type === 'grid' && (
                        <div id={`render-grid-${el.id}`} className="grid grid-cols-2 gap-6 w-full">
                          {(el.gridItems || []).map((col) => (
                            ...
                            >
                              <h4 className="font-bold" style={{ fontSize: `${Math.max(MIN_FONT_SIZE, el.fontSize * 1.15)}px`, color: theme.accent }}>
                                {col.title || 'Grid Heading'}
                              </h4>
                              <p className="opacity-90 font-medium" style={{ fontSize: `${Math.max(MIN_FONT_SIZE, el.fontSize)}px`, color: theme.text }}>
                                {col.desc || 'Grid detailed information description.'}
                              </p>
                            </div>
                          ))}
                        </div>
                        ...
                        >
                          {el.imageUrl ? (
                            ...
                            >
                              <img
                                src={el.imageUrl}
                                alt={el.imageAlt || 'Accessible presentation graphic'}
                            ...
                            >
                              <span className="text-[11px] font-bold uppercase tracking-wide opacity-50">No Image Loaded</span>
                              <span className="text-[9px] opacity-40 text-center">Use the Canvas Inspector to upload or choose an image</span>
                            </div>
                          )}
                        </div>
                      )}
          ...
          <div id="slide-metadata-footer" className="flex items-center justify-between text-[11px] font-bold uppercase opacity-60 tracking-wider mt-2 border-t pt-2" style={{ borderColor: `${theme.accent}30` }}>
            <span className="truncate max-w-[250px]">{metaCreator || formatPublishedDate(new Date())}</span>
            <span>SLIDE {activeSlideIndex + 1} OF {slides.length}</span>
          </div>

          {/* Drag over Visual Overlay */}
          {isDragOver && (
            <div id="drag-over-overlay" className="absolute inset-0 bg-indigo-600/10 backdrop-blur-[1px] border-4 border-dashed border-indigo-500 rounded-2xl flex flex-col items-center justify-center gap-2 z-50 select-none pointer-events-none animate-pulse">
      ...
      <div id="deck-filmstrip" className="w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl bg-white border border-slate-200 rounded-xl p-3 flex flex-col gap-2 shadow-sm shrink-0">
        <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
            <Sparkles className="w-3.5 h-3.5 text-indigo-500 animate-pulse" /> Widescreen Presentation Filmstrip
          </span>
          <button
        ...
        <div id="filmstrip-slides-list" className="flex items-center gap-2 overflow-x-auto py-1.5 px-2">
            ...
            return (
              ...
              >
                <div className="text-[7px] font-bold truncate max-w-[65px] opacity-70">
                  {slide.elements[0]?.content || slide.elements[0]?.title || `Slide ${idx + 1}`}
                </div>
