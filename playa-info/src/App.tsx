    ...
    },
    {
      ...
      elements: [
        ...
        },
        {
          id: 'stats-desc',
          type: 'paragraph',
          content: 'When content is displayed on overhead auditorium projectors or kiosk displays, standard 12-14pt body text is completely unreadable. Enforcing 20pt+ font size guarantees general legibility.',
          fontSize: 20,
          align: 'left'
        }
      ]
...
export default function App() {
      ...
      slides: [
        {
          ...
          elements: [
            ...
            },
            {
              id: `el-${Date.now()}-p`,
              type: 'paragraph',
              content: 'Modify this paragraph text. Enforces strict visual accessibility standards.',
              fontSize: 22,
              align: 'center'
            }
          ]
    ...
    switch (type) {
      ...
      case 'paragraph':
        defaultEl = {
          id: elId,
          type: 'paragraph',
          content: 'Add structured narrative details here. Content is optimized for readable font display.',
          fontSize: 20,
          align: 'left'
        };
        break;
      ...
      case 'quote':
        defaultEl = {
          id: elId,
          type: 'quote',
          content: 'Inclusive design means considering the full range of human diversity.',
          fontSize: 22,
          align: 'left'
        };
        break;
      ...
      case 'grid':
        defaultEl = {
          ...
          type: 'grid',
          gridItems: [
            { id: '1', title: 'Feature Alpha', desc: 'Detail specifications locked to 20pt.' },
            { id: '2', title: 'Feature Beta', desc: 'Second column details aligned.' }
          ],
          fontSize: 20,
          align: 'left'
        };
      ...
      case 'stats':
        elements = [
          ...
          },
          {
            id: `st-p-${Date.now()}`,
            type: 'paragraph',
            content: 'Our system enforces standard sizing constraints to eliminate low-contrast micro-copy entirely from executive presentations.',
            fontSize: 20,
            align: 'left'
          }
        ];
      ...
      case 'quote':
        elements = [
          ...
          },
          {
            id: `qt-q-${Date.now()}`,
            type: 'quote',
            content: 'Accessibility is not an afterthought; it is the absolute foundation of professional slide design.',
            fontSize: 24,
            align: 'center'
          }
        ];
      ...
      case 'comparison':
        elements = [
          ...
          },
          {
            ...
            type: 'grid',
            gridItems: [
              { id: '1', title: 'Display Mode', desc: 'Enforces readable 16:9 sizing patterns to minimize squished pages.' },
              { id: '2', title: 'Export Engine', desc: 'Direct compilation to high-DPI landscape printable PDFs.' }
            ],
            fontSize: 20,
            align: 'left'
          }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const metadataToSave = {
        ...
        isArchived: activePresentation.metadata.expiresAt ? new Date(activePresentation.metadata.expiresAt).getTime() <= Date.now() : false,
        exportedAt: new Date().toISOString()
      };
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(metadataToSave, null, 2));
      ...
      alert('Failed to compile your slides into a PDF file. Please verify elements are configured properly.');
    } finally {
  ...
  if (presentations.length === 0) {
    return (
      <div id="loading-spinner-app" className="flex items-center justify-center h-screen bg-slate-50 text-sm font-semibold text-slate-500 font-sans">
  ...
  const isFirstElement = activeElements.indexOf(selectedElement as SlideElement) === 0;
  const isLastElement = activeElements.indexOf(selectedElement as SlideElement) === activeElements.length - 1;
  ...
  return (
    <div id="app-root-layout" className="flex h-screen w-screen bg-white font-sans text-slate-800 overflow-hidden">
      ...
      <div id="canvas-wrapper-pane" className="flex-1 flex flex-col h-full relative" onClick={() => setSelectedElementId(null)}>
        ...
        {isArchived && (
          <div id="archive-locked-notice-strip" className="w-full bg-amber-500 text-white font-bold py-2.5 px-4 flex items-center justify-center gap-2 text-xs shadow-md shrink-0 select-none z-20">
            ...
            <span>🔒 DOCUMENT ARCHIVED: THIS DECK HAS EXPIRED & BECOME READ-ONLY. TO RESUME EDITING, EXTEND EXPIRES AT SETTINGS.</span>
          </div>
        )}
      ...
      {isExporting && exportProgress && (
        <div id="export-progress-modal" className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 flex flex-col items-center text-center space-y-4">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wide">Compiling Accessible Slide PDF</h3>
